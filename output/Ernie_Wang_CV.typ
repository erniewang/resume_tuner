...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Established endpoints on an audio file's playbackrate, volume, and pitch that listen to tempo changes via websockets \(Socket.io\)],[Architected a distributed software utilizing Node.js for reciving and inteperting data and Flask \(python\) to run the music generation model],[Engineered real-time tempo approximator that measures the period of movement by running a FFT over the captured velocity data over a specified time interval],[Performed linear interpolation and outlier filtering on captured posedata to smooth out and standardize approximated velocity, significantly improving consistency],[Developed a adjustable parameters such as normilization, and frames per second that allow users on the frontend to change the sensitivieties of input and capture rate to match the dancer],)
  ],
)
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Designed a multipaged UI to create, modify, and save a megaresume utilizing Electron's IPC utilizing React and TailwindCSS],[Cut token usage nearly 50\% by directing LLMs to reply with indexed outputs instead of echoing input context.],[Designed and implemented interactive suggestion review system in the UI with Accept\/Reject\/Edit options for each AI-generated resume improvement.],[Broaded the project to utilize plugin-based architecture supporting multiple AI providers with encrypted local storage utilizing node-keytar and model controll in the UI],)
  ],
)
