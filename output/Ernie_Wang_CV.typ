...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Designed and implemented a browser application utilizing PoseNet\(p5.js\/m15\) to caputure and analyze dance pases via a webcamera],[Engineered real-time tempo approximator that measures the period of movement by running a FFT over the captured velocity data over a specified time interval],[Established endpoints on an audio file's playbackrate, volume, and pitch that listen to tempo changes via websockets \(Socket.io\)],[Integrated with a generative music model \(Vampnet\) to continously generate new music based off of select characteristics and patterns in captured posenet data],[Developed a adjustable parameters such as normilization, and frames per second that allow users on the frontend to change the sensitivieties of input and capture rate to match the dancer],)
  ],
)
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Stored user-defined presets in browser local storage alongside providing a few well known haromization algorithms from well known music arrangers\(Count Basie, Stan Kenton, Thad Jones\)],[Built a customizable fallback algorithm that harmonizes on a more consistent basis on note\/harmony parings that were not successfull in the intial run],[Implemented real-time server streaming to highlight exceptional notes and provide interactive harmonization options, allowing users to customize non-trivial chord voicings on the fly],[Improved runtime speed of music XML parsing\/modifying programs 9 times by caching runtime results in a json file that is eventually stored on the users localStorage.],)
  ],
)
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Designed a layed, multipaged UI to create, modify, and save a megaresume utilizing Electron's IPC utilizing React and TailwindCSS],[Optimized LLM performance by implementing section-based task queuing and indexed outputs, reducing token usage by 50\% and enabling granular resume optimization],[Developing a optional keyword extration preprocessing stage that selects keywords and verbs and `upgrades` them to match keywords of the job description],[Designed and implemented interactive suggestion review system in the UI with Accept\/Reject\/Edit options for each AI-generated resume improvement.],[Broaded the project to utilize plugin-based architecture supporting multiple AI providers with encrypted local storage utilizing node-keytar and model controll in the UI],)
  ],
)
